...
class Mnemosyne
  ...
  class << self
    ...
    # Re-open the memory db at the (possibly changed) CONFIG.memory_db_path and
    # clear the in-memory Aegis orientation. The daemon calls this on
    # `activate_context` so a context switch moves persisted memory to the new
    # project's `.aether/mnemosyne.db` without restarting the process.
    def reset_connection!
      ...
      @db = nil
      self.aegis = { tags: [], summary: '', temperature: 1.0, working_dir: nil, thinking: nil }
      true
    end
    ...
    def migrate(db)
      ...
      if 7 > db_version
        ...
        db.execute 'ALTER TABLE project_notes ADD COLUMN facet TEXT' unless existing_note_columns.include?('facet')
        db.execute 'ALTER TABLE project_notes ADD COLUMN updated_at DATETIME' unless existing_note_columns.include?('updated_at')
        ...
        db.execute "INSERT OR REPLACE INTO meta (key, value) VALUES ('db_version', '7')"
      end

      if 8 > db_version
        existing_note_columns = db.execute('PRAGMA table_info(project_notes)').map { |col| col['name'] }
        db.execute 'ALTER TABLE project_notes ADD COLUMN context TEXT' unless existing_note_columns.include?('context')
        db.execute 'ALTER TABLE project_notes ADD COLUMN source_note_id INTEGER' unless existing_note_columns.include?('source_note_id')
        db.execute "INSERT OR REPLACE INTO meta (key, value) VALUES ('db_version', '8')"
      end

      # Version 9 — Salomo Rex: das Siegel. A plan ledger above the task engine.
      # `seals` is the goal/brief; `tasks` gain the executive fields (owner =
      # delegated companion, seal_id = membership, depends_on = dependency edges,
      # milestone_order = critical-path sequence).
      if 9 > db_version
        db.execute <<~SQL
        ...
        existing_task_columns = db.execute('PRAGMA table_info(tasks)').map { |col| col['name'] }
        db.execute 'ALTER TABLE tasks ADD COLUMN owner TEXT' unless existing_task_columns.include?('owner')
        db.execute 'ALTER TABLE tasks ADD COLUMN seal_id INTEGER' unless existing_task_columns.include?('seal_id')
        db.execute "ALTER TABLE tasks ADD COLUMN depends_on TEXT DEFAULT '[]'" unless existing_task_columns.include?('depends_on')
    ...
    def set_aegis_thinking(thinking) = Aegis.set_aegis_thinking(thinking)
    ...
    def companion_release(previous) = Companion.release(previous)
