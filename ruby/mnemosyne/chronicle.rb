...
class Mnemosyne
  ...
  class Chronicle
    class << self
        ...
        # Delete the most recent chronicle entry (prompt + answer) — used by the
        # frontend when a prompt is edited, deleted, or retried so the retry does
        # not leave a stale or duplicated exchange in history.
        def delete_last_entry
          Mnemosyne.db.execute \
            'DELETE FROM entries WHERE id = (SELECT id FROM entries ORDER BY id DESC LIMIT 1)'
          Mnemosyne.db.changes.positive?
        end
