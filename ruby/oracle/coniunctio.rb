...
module Coniunctio
  ...
  class << self
    def build(params)
      ...
      # Metempsychosis: merge the origin context's transmitted memories so the oracle
      # works with the combined memories of both contexts.
      injected = params[:injected_notes]
      if injected.present?
        injected = [injected] if injected.is_a?(Hash)
        aegis_notes = (aegis_notes + injected).reject(&:nil?).uniq
      end

      ephemeral = params[:ephemeral]
      ctx = { history:       prepend_summaries(history),
              extra_context: build_extra_context(attachments, project_summary, aegis_notes,
                                                 params[:context], ephemeral:)
                  .merge(flash_auto: params[:flash_auto],
    ...
    def build_extra_context(attachments, project_summary, aegis_notes, context = nil, ephemeral: false)
      # Ephemeral turns (flash conjuration) keep only stable project context —
      # the volatile Aegis orientation/notes and the manifest are omitted so the
      # one-shot never inhales stale oracle state (e.g. a lingering temperature).
      hermetic_manifest = ephemeral ? nil : read_hermetic_manifest
      ...
      { project_summary:,
        attachments:,
        aegis_orientation: ephemeral ? nil : { **Mnemosyne.aegis },
        aegis_notes:       ephemeral ? nil : aegis_notes,
