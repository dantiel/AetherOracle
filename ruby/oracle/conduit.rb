...
class Conduit
  ...
  class << self
    ...
    def use_thinking_mode?(_config, _reasoning)
      thinking = Mnemosyne.aegis[:thinking].to_s
      thinking = 'normal' if thinking.empty?
      THINKING_LEVELS.take(3).include?(thinking)
    end
    ...
    # `reasoning-model` may be a boolean flag ("yes"/"true"/"no") OR a concrete
    # model name (e.g. "moonshotai.kimi-k2-thinking"). Treat flag-like values as
    # nil so they never leak into `select_model` as a model identifier.
    def reasoning_model_config
      rm = CONFIG::CFG[:reasoning_model] || CONFIG::CFG['reasoning-model']
      return nil if rm.nil? || rm == true || rm == false
      return nil if %w[yes no true false].include?(rm.to_s.downcase)
      rm
    end

    def is_reasoning_model?(model_name)
      return false if reasoning_model_config.nil?
      is_deepseek_reasoning_model?(model_name) || reasoning_model_config.to_s == model_name.to_s
    end


    def select_max_tokens(reasoning)
      # Thinking mode (DeepSeek/Anthropic extended reasoning) returns reasoning_content
      # alongside content — both count against max_tokens. 8K would truncate the chain of
      # thought and garble the answer, so reserve the full budget whenever thinking is on.
      (reasoning || use_thinking_mode?(CONFIG::CFG, reasoning)) ? 64_000 : 8192
    rescue StandardError => e
    ...
    def extract_api_key(config)
      # Precedence: explicit config hash → the merged CONFIG (which reads
    ...
    def build_body_with_messages_and_tools(messages, tools: nil, reasoning: false, temperature: nil)
      ...
      if use_thinking_mode?(CONFIG::CFG, reasoning) && :gemini != CONFIG::CFG[:api_type]&.to_sym
        # DeepSeek (OpenAI format) and Anthropic both accept extended thinking via
        # `thinking`/`reasoning_effort`. Thinking mode ignores temperature, so drop it.
      ...
      elsif reasoning || is_reasoning_model?(model)
    ...
    # any assistant message that was produced while thinking was enabled. The
    # Swift chat history only carries role/content, and the empty-response guard
    # synthesizes assistant placeholders without reasoning — both paths can strip
    # the field and trip a 400. Normalize every assistant message so the field is
    # always present (empty string when unknown) whenever thinking mode is on.
    def normalize_reasoning_messages(messages, thinking:)
      return messages unless thinking

      messages.map do |msg|
        role = msg[:role] || msg['role']
        next msg unless role == 'assistant'

        # Round-tripped client history uses string keys; the daemon's own loop
        # messages use symbol keys. Normalize whichever key shape is present.
        if msg.key?(:reasoning_content)
          next msg if msg[:reasoning_content].is_a?(String)

          m = msg.dup
          m[:reasoning_content] = msg[:reasoning_content].to_s
          m
        elsif msg.key?('reasoning_content')
          next msg if msg['reasoning_content'].is_a?(String)
          ...
          # Field absent — add an empty one so DeepSeek never sees an assistant
          # message that "should" carry reasoning_content without it.
          m = msg.dup
          m[:reasoning_content] = ''
          m
        end
    ...
    def build_body_for_execution(messages: nil,
      ...
      thinking_mode = use_thinking_mode?(CONFIG::CFG, reasoning) && :gemini != CONFIG::CFG[:api_type]&.to_sym
      ...
      messages = normalize_reasoning_messages(messages, thinking: thinking_mode) if messages

      base_body = { model:, messages:, max_tokens:, temperature: }

      enforce_payload_limits base_body.merge(if thinking_mode
        # DeepSeek (OpenAI format) and Anthropic both accept extended thinking via
        # `thinking`/`reasoning_effort`. Thinking mode ignores temperature, so drop it.
        effort = reasoning_effort_from_aegis
        body = { tools:, thinking: { type: 'enabled' } }
 ...
 # !> assigned but unused variable - max_tokens
    ...
    def extract_response_data(json, arts)
      ...
      # Always reset — a tool-call follow-up often returns no reasoning_content,
      # and a stale value from the previous loop iteration would be attached to
      # the wrong assistant message and passed back to DeepSeek.
      arts[:reasoning_content] = msg['reasoning_content'].to_s
      [content, tcalls, arts]
    rescue StandardError => e
