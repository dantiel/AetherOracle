...
class PrimaMateria
  ...
  def select(*tool_names)
    # Create a new PrimaMateria instance keeping ONLY the named tools
    filtered_prima = PrimaMateria.new
    names = tool_names.map(&:to_sym)
  ...
  def tool_aliases
    {
      ...
      'tmp_create_file'   => 'temp_create_file'
    }
  end


  def handle(tool:, args: {}, context: nil, timeout: nil)
    tool = tool.to_s
    tool = tool_aliases[tool] || tool
    args = symbolize(args || {})

    # Leihfrist §7: each instrument holds a lease. run_command honours the
    # caller's explicit subprocess timeout; every other instrument is bounded by
    # its declared schema `timeout:`, else the æther's default patience. A silent
    # native hang must not starve the single serial worker FIFO.
    tool_timeout = resolve_lease(tool, args, timeout)
  ...
  # Resolve the lease (Leihfrist) for an instrument. The æther lends each
  # instrument a bounded window so a silent native hang cannot starve the one
  # serial heart-chamber (the worker FIFO). run_command keeps its explicit
  # subprocess timeout; long conjurations may declare a longer lease.
  def resolve_lease(tool, args, fallback)
    declared = @tools.key?(tool.to_sym) ? @tools[tool.to_sym].timeout : nil
    explicit = args[:timeout]

    if tool.to_s == 'run_command'
      # run_command's true bound is its subprocess timeout (param default 30s),
      # not the schema `timeout:` field. The æther's patience is only a net.
      explicit || fallback || DEFAULT_LEASE
    else
      declared || explicit || fallback || DEFAULT_LEASE
    end
  end
  ...
  ALLOW_CMDS   = [/^rspec\b/, /^rubocop\b/, /^git\b/, /^ls\b/, /^cat\b/, /^mkdir\b/,
                  ...
                  /^oc\b/, /^file\b/, /^hexdump\b/, %r{^/usr/sbin/screencapture\b}].freeze
  DENY_PATHS   = [/\.aethercodex$/, /\.env$/, %r{\.git/}].freeze
  MAX_DIFF     = 800
  MAX_CMD_TIME = 10
  DEFAULT_LEASE = 60 # seconds — the æther's default patience for any instrument
