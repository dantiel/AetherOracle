...
# Thinking levels per step: flash ("fast") for broad exploration, max for critical precision
STEP_THINKING = {
  1  => 'fast',    # Nigredo: Flash model for broad initial investigation
  2  => 'fast',    # Albedo: Fast creative solution design
  3  => 'normal',  # Citrinitas: Balanced exploration of implementation paths
  4  => 'max',     # Rubedo: Selecting the stone — critical decision point
  5  => 'high',    # Solve: Identifying code changes precisely
  6  => 'max',     # Coagula: Precise implementation — deepest reasoning
  7  => 'normal',  # Test: Deterministic testing
  8  => 'high',    # Purificatio: Edge case probing needs thoroughness
  9  => 'max',     # Validatio: Security/performance audit requires max thinking
  10 => 'normal'   # Documentatio: Balanced documentation
}.freeze
...
class MagnumOpusEngine
  ...
  def evaluate_task(task_id)
    ...
    # current_step is last completed step (1-indexed), 0 = none done
    current_step = self.current_step task_id
    next_step = [current_step + 1, max_steps].min
    phase_name = alchemical_phase_name(next_step)
...

    {
      ...
      task:                   {
        ...
        progress_percentage: ((current_step.to_f / max_steps) * 100).round(1),
      ...
      alchemical_progression: STEP_PURPOSES.first(max_steps).map.with_index do |purpose, idx|
        {
          stage:     alchemical_phase_name(idx + 1),
          step:      idx + 1,
          purpose:   purpose,
          completed: idx < current_step,
          current:   idx == current_step,
          result:    step_results[(idx + 1).to_s]
        }
      end
    }
  end
  ...
  # Build a rich, UI-ready snapshot of a task — every alchemical phase with its
  # name, purpose, completion, result, rejection and tool-call count, plus the
  # execution logs and sub-tasks. Read-only (never conjures the oracle), so the
  # frontend can poll it live while the worker thread drives the task.
  def task_detail(task_id)
    ...
    current = (task[:current_step] || 0).to_i
    status = task[:status] || 'pending'

    step_results = load_step_results task_id
    tool_calls = safe_parse_json(task[:tool_calls_json], {})
    tool_calls = {} unless tool_calls.is_a?(Hash)

    logs = safe_parse_json(task[:logs], [])
    logs = [] unless logs.is_a?(Array)

    steps = (1..max_steps).map do |i|
      phase = alchemical_phase_name i
      name = @mnemosyne.step_name workflow_type, i
      raw = step_results[i.to_s].to_s
      # Step results are stored as highlighted HTML; the frontend wants plain text
      # and rejection/failure detection needs the un-mangled prefix.
      plain = raw.gsub(/<[^>]*>/, '')
                 .gsub('&lt;', '<').gsub('&gt;', '>').gsub('&amp;', '&')
                 .gsub('&quot;', '"').gsub('&#39;', "'").strip
      rejected = plain.start_with?('REJECTED:')
      failed = plain.start_with?('ERROR', 'MAX_RETRIES_EXCEEDED', 'NO_COMPLETION_SIGNAL')
      tc = tool_calls[phase.to_sym] || tool_calls[phase.to_s] || []
      # Compact instrumenta ledger per phase: name + args + result preview, so the
      # frontend can render the calls the model made during this alchemical step.
      # `result` is truncated to keep the detail frame light; the full result stays
      # in `step_results` for the expanded phase view.
      tc_compact = if tc.respond_to?(:map)
                     tc.map do |call|
                       call = call.transform_keys(&:to_s) if call.respond_to?(:transform_keys)
                       {
                         name:           call['name'] || call[:name],
                         args:           call['args'] || call[:args],
                         result:         (call['result'] || call[:result]).to_s.truncate(400),
                         execution_time: call['execution_time'] || call[:execution_time]
                       }
                     end
                   else
                     []
                   end
      {
        ...
        name:             name,
        phase:            phase.to_s,
        purpose:          STEP_PURPOSES[i - 1] || name,
        completed:        i <= current && 'pending' != status && !rejected && !failed,
        active:           i == current + 1 && %w[active].include?(status),
        rejected:         rejected,
        rejection_reason: rejected ? plain.sub(/\AREJECTED:\s*/, '') : nil,
        failed:           failed,
        result:           plain.empty? ? nil : plain,
        tool_call_count:  tc.respond_to?(:size) ? tc.size : 0,
        tool_calls:       tc_compact
      }
    end

    subtasks = (@mnemosyne.get_subtasks(task_id) || []).map do |s|
  ...
  def execute_task(task_id)
    ...
    # Re-fetch task state — execute_task_internal may have set :failed
    task = @mnemosyne.get_task task_id
    return if task && 'failed' == task[:status]
  ...
  # @param [String, nil] retry_guidance Escalating retry message injected into user prompt.
  # @raise [RuntimeError] If the oracle's response is invalid or times out.
  def execute_step(task_id, step_index, workflow_type = 'full', retry_guidance: nil)
    ...
    system_messages = [
      ...
      <<~STEP_GUIDANCE,
        ...
        Current temperature: #{STEP_TEMPERATURES[step_index] || 1.0} (#{4 >= step_index ? 'Exploration - Higher creativity' : 'Implementation - Precise execution'}). Thinking: #{STEP_THINKING[step_index] || 'normal'}.#{STEP_THINKING[step_index] == 'fast' ? ' Flash model — broad investigation, escalate via aegis(thinking: "normal") if needed.' : ''} You may override via `aegis(thinking: "X")` or `aegis(temperature: X)` if this phase needs a different setting.
      ...
      <<~EXECUTION_INSTRUCTIONS
        ...
        #{case workflow_type
          when 'debug'
            <<~DEBUG_INSTRUCTIONS
              # DEBUG WORKFLOW — PHASE STEP-THROUGH (NO TOOLS)
              - You are running a DEBUG task that merely steps through the alchemical phases.
              - NO tools are available. Do NOT call any tool — simply reason about this phase.
              - Respond in plain text describing what this phase would entail for the task.
              - The engine advances automatically after your response; you do NOT need
                task_complete_step or task_reject_step.
              - Keep the response focused on the current phase's purpose.
            DEBUG_INSTRUCTIONS
          when 'simple'
            if 1 >= step_index
              <<~SIMPLE_ANALYZE
    ...
    begin
      # Set temperature and thinking based on step phase
      step_temperature = STEP_TEMPERATURES[step_index] || 1.0
      step_thinking = STEP_THINKING[step_index] || 'normal'
      ...
      @mnemosyne.set_aegis_thinking step_thinking
      ...
      # Debug workflow: no tools at all — the model reasons through each phase in
      # plain text and the engine auto-advances on a successful response.
      tools = 'debug' == workflow_type ? PrimaMateria.new : create_task_tools(task_id)
      ...
      begin
        ...
        case workflow_type
        when 'debug'
          # No tool filtering needed — tools is already empty.
        when 'simple'
          if 1 == step_index
      ...
      when :__divine_interrupt_step_completed
        ...
        if result.to_s.strip.empty?
          # The model signalled completion but no result arrived — usually a
          # truncated/malformed task_complete_step argument. Never mask this as a
          # cryptic sentinel; surface a meaningful message so the loss is visible.
          HorologiumAeternum.system_error \
            "Step #{step_index} signalled completion without a result (task_complete_step argument was empty or truncated)"
          result = 'Step completed (no result captured — see system log)'
        end
      ...
      when :success, 'success', :step_completed, 'step_completed'
        ...
        # A plain successful response means the model finished reasoning this step
        # WITHOUT calling task_complete_step. That is the normal behaviour of the
        # DeepSeek chat model (it narrates its conclusion instead of emitting a tool
        # completion signal), so we must treat it as step completion and auto-advance.
        # Requiring the explicit tool here is what made every task fail with
        # NO_COMPLETION_SIGNAL after exhausting retries — the model's final answer
        # was discarded and the step re-run up to four times for nothing.
        result = answer.to_s.strip.empty? ? 'Step completed (no narrated result)' : answer
  ...
  # Escalating retry guidance messages — each attempt gets more prescriptive.
  RETRY_GUIDANCE = [
    # attempt 2 (first retry)
    <<~RETRY1,
      \n\n=======\n
      # ⚠️ RETRY GUIDANCE (Attempt 2 of #{MAX_STEP_RETRIES})
      Your previous execution of this step did not produce a valid result.
      You MUST call task_complete_step(result) with your findings to advance.
      If you cannot complete the step, call task_reject_step(reason, restart_from_step: N)
      to restart from an earlier phase. Do NOT proceed without calling one of these.
      The tools task_complete_step and task_reject_step are the ONLY way to progress.
    RETRY1
    # attempt 3 (second retry)
    <<~RETRY2,
      \n\n=======\n
      # 🔴 URGENT RETRY GUIDANCE (Attempt 3 of #{MAX_STEP_RETRIES} — LAST ATTEMPT)
      This is your FINAL attempt. The previous two executions failed to produce a step completion signal.
      You MUST use task_complete_step(result: "...") or task_reject_step(reason: "...") IMMEDIATELY.
      Failure to do so will mark this task as FAILED and all work will be lost.
      Focus ONLY on producing a completion signal — even a partial result is better than nothing.
      If you are unsure, call task_reject_step(reason: "uncertain", restart_from_step: 1) to restart.
    RETRY2
    # attempt 4 (third retry — max reached, but try once more)
    <<~RETRY3
      \n\n=======\n
      # 🛑 FINAL RETRY (Attempt #{MAX_STEP_RETRIES + 1} — TASK WILL BE MARKED FAILED IF UNSUCCESSFUL)
      You have exhausted all retries. Call task_complete_step or task_reject_step NOW
      or this task will be permanently failed. Any partial result is acceptable.
      Save whatever you have with task_complete_step(result: "partial: ...").
    RETRY3
  ].freeze

  # Internal execution method — executes ONE step with up to MAX_STEP_RETRIES retries.
  def execute_task_internal(task_id, current_step, workflow_type, max_steps)
    current_step = [[current_step, 0].max, max_steps].min
    return if halted?(task_id) || current_step >= max_steps

    step_num = current_step + 1 # 1-indexed for display
    last_error = nil

    (0..MAX_STEP_RETRIES).each do |attempt|
      begin
        retry_guidance = attempt.zero? ? nil : RETRY_GUIDANCE[[attempt - 1, RETRY_GUIDANCE.size - 1].min]
        step_result = if retry_guidance
                        execute_step task_id, step_num, workflow_type, retry_guidance: retry_guidance
                      else
                        execute_step task_id, step_num, workflow_type
                      end

        # ── Divine interruption: step_completed ──
        if step_result.is_a?(Hash) && :step_completed == step_result[:__divine_interrupt]
          log_message task_id, "Step #{step_num} completed via divine interruption"
          store_step_result task_id, step_num, step_result[:result] if step_result[:result]
          next_step = [step_num + 1, max_steps + 1].min
          update_progress task_id, next_step - 1
          task = @mnemosyne.get_task task_id
          HorologiumAeternum.task_step_completed(**task, result: step_result[:result]) if task
          return execute_task_internal task_id, current_step(task_id), workflow_type, max_steps
        end

        # ── Divine interruption: step_rejected ──
        if step_result.is_a?(Hash) && :step_rejected == step_result[:__divine_interrupt]
          log_message task_id, "Step #{step_num} rejected: #{step_result[:reason]}"
          store_step_result task_id, step_num, "REJECTED: #{step_result[:reason]}"
          restart_step = [[step_result[:restart_from_step] || current_step, 1].max, max_steps].min
          update_progress task_id, restart_step - 1
          task = @mnemosyne.get_task task_id
          HorologiumAeternum.task_step_rejected(**task, reason: step_result[:reason]) if task
          return execute_task_internal task_id, current_step(task_id), workflow_type, max_steps
        end

        # ── Legacy success path (shouldn't happen, but handle) ──
        if step_result.is_a?(Hash) && :step_completed == step_result[:status]
          log_message task_id, "Step #{step_num} completed (legacy path)"
          answer = step_result[:response] && step_result[:response][:answer]
          store_step_result task_id, step_num, answer if answer
          next_step = [step_num + 1, max_steps].min
          update_progress task_id, next_step - 1
          begin
            execute_task_internal task_id, next_step, workflow_type, max_steps
          rescue StandardError => e
            log_message task_id, "Recursive step error: #{e.message}"
          end
          return
        end

        # ── NO_COMPLETION_SIGNAL — retry ──
        last_error = 'NO_COMPLETION_SIGNAL: Step executed but no task_complete_step called'
        log_message task_id, "Step #{step_num} attempt #{attempt + 1}: #{last_error}"
        store_step_result task_id, step_num, "#{last_error} (attempt #{attempt + 1}/#{MAX_STEP_RETRIES + 1})"
        # Fall through to next retry

      rescue MagnumOpusEngine::TaskStateError, StandardError, Timeout::Error => e
        last_error = "#{e.class}: #{e.message}"
        log_message task_id, "Step #{step_num} attempt #{attempt + 1}: #{last_error}"
        store_step_result task_id, step_num, "ERROR (attempt #{attempt + 1}/#{MAX_STEP_RETRIES + 1}): #{e.message}"
        # Fall through to next retry
      end
    end

    # ── All retries exhausted ──
    log_message task_id, "Step #{step_num} FAILED after #{MAX_STEP_RETRIES + 1} attempts: #{last_error}"
    store_step_result task_id, step_num,
                      "MAX_RETRIES_EXCEEDED: #{last_error} after #{MAX_STEP_RETRIES + 1} attempts"
    update_state task_id, :failed
    nil
  end
